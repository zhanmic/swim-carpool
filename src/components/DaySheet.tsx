...
interface DaySheetProps {
  ...
  onSkip: (action: "mark" | "clear", familyId?: string) => Promise<void>;
}
...
function buildScheduleSnapshot(
  ...
  return {
    ...
    no_practice: session.no_practice,
  };
}
...
export function DaySheet({
  ...
  const [noPractice, setNoPractice] = useState(session.no_practice);
  ...
  const otherFamilies = useMemo(
    () => sortedFamilies.filter((family) => family.id !== activeFamilyId),
    [sortedFamilies, activeFamilyId]
  );
  ...
  const currentSchedule = useMemo<ScheduleSnapshot>(
    ...
    [startTime, endTime, locationName, locationNotes, dropoffPickups, dropoffFamilyId, cancelled, noPractice]
  );
  ...
  async function handleSkipOther(familyId: string) {
    if (skipBusy) return;
    setSkipBusy(true);
    try {
      await onSkip(skipIds.has(familyId) ? "clear" : "mark", familyId);
    } finally {
      setSkipBusy(false);
    }
  }
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <label className="flex shrink-0 items-center gap-1 rounded-full border border-slate-200 px-2 py-0.5 text-[11px] font-medium text-slate-600 dark:border-slate-600 dark:text-slate-400">
                <input
                  ...
                  checked={noPractice}
                  onChange={(e) => {
                    setNoPractice(e.target.checked);
                    if (e.target.checked) {
                      setCancelled(false);
                    }
                  }}
          ...
          <div className="rounded-lg border border-violet-200 bg-violet-50/60 p-2 min-w-0 dark:border-violet-800 dark:bg-violet-950/40">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                {!cancelled && !noPractice && familiesNeedingPickup.length > 0 && (
            ...
            <label className={`block ${!cancelled && !noPractice && familiesNeedingPickup.length > 0 ? "mt-1.5" : "mt-1"}`}>
